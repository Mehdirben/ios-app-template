...
\subsubsection*{2. Prevent GNOME/KDE from Locking the iPhone on Boot}
\begin{importantbox}[Volume Monitor Conflict Warning]
Desktop environments (like GNOME) run background volume monitors (\path{gvfs-gphoto2-volume-monitor} and \path{gvfs-afc-volume-monitor}) that automatically seize the iPhone's USB endpoint and lockdown socket on boot. This triggers \texttt{lockdown error -8} or device-busy conflicts and freezes \texttt{usbmuxd}.
...
\subsubsection*{3. Pair and Trust Your iPhone}
\begin{enumerate}
...
\subsubsection*{4. Install Impactor via Flathub (Recommended)}
...
\subsubsection*{5. Install the App using Impactor}
\begin{enumerate}
...
\subsubsection*{2. GNOME Monitors Conflict (\texttt{gvfs-gphoto2} \& \texttt{gvfs-afc})}
...
\subsubsection*{3. Impactor Opened Before Phone Connected}
